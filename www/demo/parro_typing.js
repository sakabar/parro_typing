...
var dispCharWindowSize = 32; //何文字先まで見せるか
...
function defaultPaint(){
    paintKeyboard();
    document.js1.errorGoalInputaa.value = errorRateGoal * 100.0;
    document.js2.speedGoalInputaa.value = charPerMinGoal;
}
...
function setSpeedGoal(){
    var input = document.js1.speedGoalInputaa.value;
    if(input > 0){
        charPerMinGoal = input;
        alert("速度の目標を「" + charPerMinGoal + "文字/分」に設定しました。");
    }
    else{
        alert("値が不正です。0より大きい数値を入力してください。");
    }
}

function setErrorGoal(){
    var input = document.js2.errorGoalInputaa.value / 100.0;
    if(0<= input && input <= 100){
        errorRateGoal = input;
        alert("誤り率の目標を「" + errorRateGoal * 100 + "%以下」に設定しました。");
    }
    else{
        alert("値が不正です。");
    }
